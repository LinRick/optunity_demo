...
def start_master(dask_scheduler=None, port=7077):
    ...
    logger.info("Spark master proc at %s", proc)
...
def spark_to_dask(sc, loop=None):
    ...
    dask_to_spark
    """
    cluster = LocalCluster(n_workers=1, loop=loop,threads_per_worker=4, 
                           scheduler_port=8786)
    ...
    print("cluster.scheduler.workers=%s" %cluster.scheduler.workers)
    
    print("cluster.scheduler.counters=%s" %cluster.scheduler.counters)
